...
function SearchContent() {
  ...
  // TODO(supabase): replace with a server query filtered by profession/city/name,
  // always scoped to the professional's own `city` column (never the customer's location).
  const results = useMemo(() => {
    return MOCK_PROFESSIONALS.filter((p) => p.status === "approved").filter((p) => {
      const matchesQuery =
        query.trim() === "" ||
  ...
  }, [query, professionFilter, cityFilter]);
